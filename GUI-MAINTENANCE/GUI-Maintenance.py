...
GUI.title(lang.menu_lang[language])
...
def save():
    ...
    tsid = str(int(datetime.now().strftime('%y%m%d%H%M%S')) )
...
#mtworkorderlist.heading('TSID',text='TSID')



for h ,w in zip (header,headerw):
    # H = TSID W= 50 -----> H=ชื่อ w 100 -----> H =แผนก  w = 150
...
#print(data)
    for d in data:
        d = list(d) #แปลง tupple เป็น list
